...
func etcdRegisterService() {
	...
	if err != nil {
		tCommon.Logger.SystemErrorLog("ERROR_HOST_IP: ", err)
		return
	}
	...
	for _, reg := range mpEtcdRegPerList {
		...
		if ENV_NODE_HOST == strRegHost {
			// 服务注册
			err = registerService(tServer.Etcd_Client, ENV_NODE_TAG, ENV_NODE_HOST)
			if err != nil {
				tCommon.Logger.SystemErrorLog("ETCD_REGISTER_ERROR: ", err)
			}
