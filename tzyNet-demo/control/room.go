...
func IsRoomExist(ctx *tCommon.ConContext, roomId uint64) *api.IsRoomExist_OutObj {
	...
	if err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_REDIS_QUERY, err)
	}
...
func CreateRoom(ctx *tCommon.ConContext, roomId uint64, actId uint32, gameLv uint32) *api.CreateRoom_OutObj {
	...
	ok, err := model.IsRoomExits(ctx, roomId)
	if ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "创建失败，房间id已存在")
	} else if err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_REDIS_QUERY, err)
	}
	...
	if arrUserInfo[uid] == nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_SDK_GET_USERINFO, "调取Sdk获取用户数据失败")
	}
	...
	if ret == nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_REDIS_WRITE_ERROR, "用户数据获取失败或创建房间时数据写入错误")
	}
...
func JoinRoom(ctx *tCommon.ConContext, roomId uint64) *api.JoinRoom_OutObj {
	...
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "要加入的房间不存在")
	}

	// 是否在房间中
	if ok, _ := model.GetRoomIndex(ctx, roomModel); ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_ALREADY_IN_ROOM, "已经在房间中，无法重复加入房间")
	}
...
func JoinGame(ctx *tCommon.ConContext, postionId uint32) *api.JoinGame_OutObj {
	...
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_ROOM, "不在房间中，无法加入游戏")
	}

	// 是否在游戏中
	ok, _ = model.GetGameIndex(ctx, roomModel)
	if ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_ALREADY_IN_GAME, "已经在游戏房间中了，无法重复加入")
	}
	...
	if postionId > uint32(actCfg.MaxPlayerNum-1) {
		tCommon.Logger.GameErrorLog(ctx, ERR_GAME_POSTION_PARAM, "错误的游戏位置参数:"+strconv.FormatUint(uint64(postionId), 10))
	}
	...
	if len(gamePlayers) == actCfg.MaxPlayerNum {
		tCommon.Logger.GameErrorLog(ctx, ERR_ROOM_PLAYERNUM_FULL, "游戏房人已满，无法加入")
	}

	// 位次是否已经有人
	if _, ok = gamePlayers[postionId]; ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_ROOM_POSTION_USING, "新位次已经被玩家占用，无法调整游戏位次")
	}
	...
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_SDK_GET_USERINFO, "SDK获取玩家数据出错")
	}
...
func ChangePos(ctx *tCommon.ConContext, newPosId uint32) *api.ChangePos_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, ctx.GetConGlobalObj().RoomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "房间不存在，无法调整游戏位次")
	}

	// 是否在房间中
	ok, gameIndex := model.GetGameIndex(ctx, roomModel)
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_ROOM, "不在房间中，无法调整游戏位次")
	}

	// 位次入参合法性判断
	cfg := model.GetActCfg(roomModel.ActId)
	if newPosId >= uint32(cfg.MaxPlayerNum) {
		tCommon.Logger.GameErrorLog(ctx, ERR_GAME_POSTION_PARAM, "游戏位次参数错误:"+strconv.FormatUint(uint64(gameIndex), 10))
	}
...
func LeaveGame(ctx *tCommon.ConContext) *api.LeaveGame_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, roomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "游戏房间不存在,无法退出房间")
	}
	...
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_GAMEROOM, "不在游戏房间中，无法退出游戏")
	}
...
func SetOrCancelPrepare(ctx *tCommon.ConContext) *api.SetOrCancelPrepare_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, ctx.GetConGlobalObj().RoomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "房间不存在，无法准备")
	}

	// 是否在房间中
	ok, gameIndex := model.GetGameIndex(ctx, roomModel)
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_ROOM, "不在房间中，无法准备/取消准备")
	}
...
func GameStart(ctx *tCommon.ConContext) *api.GameStart_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, roomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "游戏房间不存在，无法开启游戏")
	}

	// 是否在游戏房间中
	ok, index := model.GetGameIndex(ctx, roomModel)
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_GAMEROOM, "不在游戏房间中，无法开始游戏")
	}

	// 是否是房主
	roomInfo := roomModel.PosIdToPlayer
	if isMaster := roomInfo[index].IsMaster; !isMaster {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_ROOM_MASATER, "不是房主，无法开启游戏")
	}
	...
	for _, player := range roomInfo {
		if player.State == false {
			tCommon.Logger.GameErrorLog(ctx, ERR_IS_NO_PREPARE_EXIST, "玩家未全部准备好，无法开启游戏")
		}
	}
...
func AddRobot(ctx *tCommon.ConContext, robotHead string, robotName string) *api.AddRobot_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, roomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "游戏房间不存在，无法创建机器人")
	}

	// 是否在游戏房间中
	ok, index := model.GetGameIndex(ctx, roomModel)
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_GAMEROOM, "不在游戏房间中，无法创建机器人")
	}

	// 是否是房主
	roomInfo := roomModel.PosIdToPlayer
	if isMaster := roomInfo[index].IsMaster; !isMaster {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_ROOM_MASATER, "不是房主，无法创建机器人")
	}
...
func DelRobot(ctx *tCommon.ConContext, robotPos uint32) *api.DelRobot_OutObj {
	...
	roomModel, err := model.GetGameRoomInfo(ctx, roomId)
	if roomModel == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, ERR_NO_ROOMID_EXIST, "游戏房间不存在，无法删除机器人")
	}

	// 是否在游戏房间中
	ok, index := model.GetGameIndex(ctx, roomModel)
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_IN_GAMEROOM, "不在游戏房间中，无法删除机器人")
	}

	// 是否是房主
	roomInfo := roomModel.PosIdToPlayer
	if isMaster := roomInfo[index].IsMaster; !isMaster {
		tCommon.Logger.GameErrorLog(ctx, ERR_IS_NOT_ROOM_MASATER, "不是房主，无法删除机器人")
	}

	if _, ok = roomModel.PosIdToPlayer[robotPos]; !ok {
		tCommon.Logger.GameErrorLog(ctx, ERR_PLAYER_ISNIT_EXIST, "该位置不是不存在玩家，无法执行删除机器人操作")
	}

	// 游戏房玩家数据
	if !roomModel.PosIdToPlayer[robotPos].IsRobot {
		tCommon.Logger.GameErrorLog(ctx, ERR_PLAYER_ISNOT_ROBOT, "该位置不是机器人，无法删除")
	}
