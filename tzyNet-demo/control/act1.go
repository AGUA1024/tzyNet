...
func Act1GameInit(ctx *tCommon.ConContext) *api.Act1Game_OutObj {
	...
	if roomInfo == nil || err != nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_NO_ROOMID_EXIST, "act1:操作的房间不存在")
	}

	// 房主是否开始游戏
	if roomInfo.IsGame == false {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_GAME_IS_NOT_STARTED, "act1:房主还未开始游戏")
	}

	// 游戏初始化
	act1model := model.NewActModel(ctx, 1)
	if act1model == nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ACTID_IS_NOT_EXIST, "act1:获取model时参数错误，actId:"+strconv.Itoa(1))
	}
...
func PlayCard(ctx *tCommon.ConContext, cardIndex uint32) *api.Act1Game_OutObj {
	act1Model := getAct1Model(ctx)

	eventType, events := act1Model.PlayCard(ctx, int(cardIndex))
	if eventType == model.EVENT_TYPE_ERROR {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ACT1_PLAYCARD_PARAM, "act1:出牌时参数错误")
	}
...
func EventHandle(ctx *tCommon.ConContext, targetIndex uint32) *api.Act1Game_OutObj {
	act1Model := getAct1Model(ctx)
	...
	if evenPlayer.Uid != ctx.GetConGlobalObj().Uid {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ACT1_CANT_PLAY, "act1:没有出牌权限")
	}
...
func GetCardFromPool(ctx *tCommon.ConContext) *api.Act1Game_OutObj {
	act1Model := getAct1Model(ctx)
	...
	if act1Info.BombPlayer != nil || ctx.GetConGlobalObj().Uid != act1Info.PlayerList[act1Info.CurPlayerIndex].Uid || act1Info.EventPlayer != nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ACT1_PLAYCARD_PARAM, "act1:无法摸牌")
	}
...
// 获取对局信息,并判断请求合法性
func getAct1Model(ctx *tCommon.ConContext) *model.Act1Model {
	// 获取房间信息
	roomInfo, err := model.GetGameRoomInfo(ctx, ctx.GetConGlobalObj().RoomId)
	if roomInfo == nil || err != nil {
	...
	// 房主是否开始游戏
	if roomInfo.IsGame == false {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_GAME_IS_NOT_STARTED, "房主还未开始游戏")
	}

	// 是否可以出牌
	actModel := model.GetActModel(ctx, 1)
	if actModel == nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_REDIS_GET_ACTINFO, "查询游戏信息失败")
	}
	...
	if act1Info.PlayerList[act1Info.CurPlayerIndex].IsDie {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ACT1_PLAYER_IS_DIE, "已经出局，无法出牌")
	}
...
// 超时提醒，报错不回包
func TurnTimeOut(ctx *tCommon.ConContext, seqId uint32) *api.Act1Game_OutObj {
	act1Model := getAct1Model(ctx)
	act1Info := act1Model.ActInfo
...
func GetAct1Info(ctx *tCommon.ConContext) *api.GetAct1Info_OutObj {
	actModel := model.GetActModel(ctx, 1)
	if actModel == nil {
