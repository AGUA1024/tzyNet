...
func GetGateWay(ctx *tCommon.ConContext, roomId uint64) *api.GetGateWay_OutObj {
	...
	if err != nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_NO_GATEWAY, "无可用网关")
	}
	...
	if err != nil {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_ETCD_GET_PORT, "从获取ETCD获取端口出错")
	}
	if len(resp.Kvs) == 0 {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_NO_GATEWAY_PORT, "找不到网关端口")
	}
...
func ConGlobalObjInit(ctx *tCommon.ConContext, cin *api.ConGlobalObjInit_InObj) *api.ConGlobalObjInit_OutObj {
	...
	if !ok {
		tCommon.Logger.GameErrorLog(ctx, tCommon.ERR_NO_CONNECT_EXIST, "与服务器的连接不存在")
	}
