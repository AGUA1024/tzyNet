...
// 创建一个新游戏
func (this *Act1Model) NewActModel(ctx *common.ConContext) ActModelInterface {
	// 获取房间信息
...
func (act *Act1Model) GetCardFromPool(ctx *common.ConContext) (uint32, []uint32) {
	// 获取游戏信息
...
// 对自己使用的卡
func (act *Act1Model) PlayCard(ctx *common.ConContext, cardIndex int) (uint32, []uint32) {
	// 获取游戏信息
	act1Info := act.ActInfo
...
// 事件处理
func (act *Act1Model) EventHandler(ctx *common.ConContext, chooseIndex uint32) (uint32, []uint32) {
	// 获取游戏信息
	act1Info := act.ActInfo
...
func playerListInit(ctx *common.ConContext, cardDeck []int) (map[uint64]uint32, map[uint32]*Player, []int) {
	roomInfo, err := GetGameRoomInfo(ctx, ctx.GetConGlobalObj().RoomId)
	if err != nil || roomInfo == nil {
		return nil, nil, nil
	}
...
// 玩家抽牌
func (act *Act1Model) playerDraw(ctx *common.ConContext) int {
	// 获取卡池
	actInfo := act.ActInfo
...
// 玩家打出手中的某张牌
func (act *Act1Model) playCard(ctx *common.ConContext, cardIndex int) int {
	actInfo := act.ActInfo
...
// 拆除炸弹
func (act *Act1Model) disarmBomb(ctx *common.ConContext, bombNewPos int) {
	actInfo := act.ActInfo
	cardpool := actInfo.CardPool
...
// 超时提醒
func (act *Act1Model) TurnTimeOut(ctx *common.ConContext) (uint32, []uint32) {
	// 获取游戏信息
	act1Info := act.ActInfo
	...
	if act1Info.BombPlayer != nil {
		...
		if isGameOver {
			...
			if back.Code != 200 {
				...
				//common.Logger.SystemErrorLog("Sdk_SendGameMsgBySdk_error：", back.Msg)
			}
...
func (act *Act1Model) RotboToDo(playerIndex uint32) {
	...
	if actInfo.PlayerList[playerIndex].IsRobot || !actInfo.PlayerList[playerIndex].IsOnline {
		...
		go func() {
			...
			fmt.Println("唤醒机器人")
			common.GlobalSysEventChan <- RobotEvent{
				fun:  RobotHandle,
				args: funArgs,
			}
		}()
...
func (act *Act1Model) DelAct(ctx *common.ConContext) {
...
func (act *Act1Model) PlayerLoseConn(ctx *common.ConContext) {
	actInfo := act.ActInfo
	playerIndex := actInfo.MpUidToIndex[ctx.GetConGlobalObj().Uid]
	player := actInfo.PlayerList[playerIndex]
	// 将状态设置为掉线
...
func (act *Act1Model) IsPlayer(ctx *common.ConContext) bool {
...
func (act *Act1Model) PlayerReConn(ctx *common.ConContext) bool {
